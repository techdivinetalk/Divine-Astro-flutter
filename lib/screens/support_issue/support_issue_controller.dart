...
class SupportIssueController extends GetxController {
  ...
  compressImages(croppedFile) async {
    ...
    if (lastIndex != -1) {
      ...
      if (result != null) {
        int imageSize = await File(result.path).length();
  ...
  Future<void> uploadImage(imageFile) async {
    ...
    request.fields.addAll({"module_name": "supportIssue"});
...
// Function to upload a list of images one by one
  ...
  submitIssues() async {
    ...
    try {
      ...
      final response = await userRepository.submitSupportIssues(param);
