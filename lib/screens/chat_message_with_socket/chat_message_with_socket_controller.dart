...
class ChatMessageWithSocketController extends GetxController
  ...
  BroadcastReceiver(names: <String>["deliveredMsg", "messageReceive","template"]);
  ...
  stateHandling() {
    ...
    _listener = AppLifecycleListener(
      ...
      onResume: () {
        ...
        print("getMessageTemplatesLocally");
        getMessageTemplates();
...
void typingListenerSocket() {
  socket.typingListenerSocket((data) {
    ...
    debugPrint(
        '${data['data']["typist"].toString()}  ${AppFirebaseService().orderData
...
Future<String?> getDownloadedImagePath(int chatDetailId,) async {
...
noticeAPi() async {
  try {
    final response = await noticeRepository.get(
        "${ApiProvider.getAstroAllNoticeType3}&orderId=${AppFirebaseService()
            .orderData.value["orderId"]
            .toString()}&userId=${AppFirebaseService()
