import 'package:flutter/material.dart';

class AppColors {
  static const Color white = Color(0xFFFFFFFF);
  static const Color darkGreen = Color(0xFF0E2339);
  static const Color lightGreen = Color(0xff27C884);
  static const Color lightYellow = Color(0xffFDD48E);
  static const Color darkYellow = Color(0xffFCB742);
  static const Color darkCoffee = Color(0xff5F3C08);
  static const Color blackColor = Colors.black;

  static const Color borderColour = Color(0xFFFDD48E);
  static const Color brownColour = Color(0xFF5F3C08);
  static const Color appRedColour = Color(0xFFEF5862);
  static const Color greyColour = Color(0xFF7E7E7E);
  static const Color lightGreyColour = Color(0xFF0E2339);
  static const Color appYellowColour = Color(0xFFFCB742);
  static const Color gradientBottom = Color(0xFFFFF2DD);
  static const Color gradientTop = Color(0xFFFCB742);

  static const Color redColor = Colors.red;
  static const Color greyColor = Colors.grey;
  static const Color lightGrey = Color(0xff87919C);
}
