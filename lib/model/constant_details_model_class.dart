...
class ConstantDetailsModelClass {
  ...
  factory ConstantDetailsModelClass.fromRawJson(String str) => ConstantDetailsModelClass.fromJson(json.decode(str));

  String toRawJson() => json.encode(toJson());

  factory ConstantDetailsModelClass.fromJson(Map<String, dynamic> json) => ConstantDetailsModelClass(
...
class Data {
  ...
  factory Data.fromJson(Map<String, dynamic> json) => Data(
    leaderboardRankImage: Map.from(json["leaderboard_rank_image"]).map((k, v) => MapEntry<String, String>(k, v)),
    lottiFile: Map.from(json["lotti_file"]).map((k, v) => MapEntry<String, String>(k, v)),
    ...
    chatSuspiciousRegex: List<String>.from(json["chat_suspicious_regex"].map((x) => x)),
    cityList: List<String>.from(json["city_list"].map((x) => x)),
    notificationType: Map.from(json["notification_type"]).map((k, v) => MapEntry<String, String>(k, v)),
    whatsappNo: json["whatsappNo"],
  );

  Map<String, dynamic> toJson() => {
    "leaderboard_rank_image": Map.from(leaderboardRankImage).map((k, v) => MapEntry<String, dynamic>(k, v)),
    "lotti_file": Map.from(lottiFile).map((k, v) => MapEntry<String, dynamic>(k, v)),
    ...
    "chat_suspicious_regex": List<dynamic>.from(chatSuspiciousRegex.map((x) => x)),
    "city_list": List<dynamic>.from(cityList.map((x) => x)),
    "notification_type": Map.from(notificationType).map((k, v) => MapEntry<String, dynamic>(k, v)),
