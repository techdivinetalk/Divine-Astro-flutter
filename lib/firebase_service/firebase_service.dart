    ...
    try {
      ...
      _database.child(path).onValue.listen((event) async {
        ...
        if (event.snapshot.value is Map<Object?, Object?>) {
          Map<String, dynamic>? realTimeData = Map<String, dynamic>.from(event.snapshot.value! as Map<Object?, Object?>);
          ...
          if (realTimeData['giftCount'] != null) {
            sendBroadcast(BroadcastMessage(name: "giftCount", data: {'giftCount': realTimeData["giftCount"]}));
            FirebaseDatabase.instance.ref("$path/giftCount").remove();
          }
          if (realTimeData['callKundli'] != null) {
            Map<String, dynamic>? callKundli = Map<String, dynamic>.from(realTimeData['callKundli'] as Map<Object?, Object?>);
          ...
          if (realTimeData["deliveredMsg"] != null) {
            sendBroadcast(BroadcastMessage(name: "deliveredMsg", data: {'deliveredMsgList': realTimeData["deliveredMsg"]}));
          }
          if (realTimeData["totalGift"] != null) {
            sendBroadcast(BroadcastMessage(name: "totalGift", data: {'totalGift': realTimeData["totalGift"]}));
          }
      ...
      if (value != "") {
        _database.child("order/$value").onValue.listen((event) async {
          ...
          if (event.snapshot.value != null) {
            Map<String, dynamic>? orderData = Map<String, dynamic>.from(event.snapshot.value! as Map<Object?, Object?>);
            ...
            if (orderData["status"] != null) {
              ...
              if (orderData.containsKey("card")) {
                print("displayCard");
                sendBroadcast(BroadcastMessage(name: "displayCard", data: null));
              }
              if (orderData["status"] == "0") {
                ...
                acceptChatRequestBottomSheet(Get.context!, onPressed: () async {
                  try {
                    if (await acceptOrRejectChat(orderId: int.parse(value.toString()), queueId: orderData["queue_id"])) {
                ...
                sendBroadcast(BroadcastMessage(name: "ReJoinChat", data: {"orderId": value, "orderData": orderData}));
